...
//            .effect((new MobEffectInstance(MobEffects.DARKNESS, 114, 514, true, false)), 1)
    ...
    public @NotNull ItemStack finishUsingItem(ItemStack itemIn, Level levelIn, LivingEntity entityIn) {
        if (levelIn instanceof ServerLevel serverLevel) {
            ...
            if (entityIn.hasEffect((Holder<MobEffect>) DysEffects.SENPAI_STARE)) {
                int level = entityIn.getEffect((Holder<MobEffect>) DysEffects.SENPAI_STARE).getAmplifier();
                if (level < 1) {
                    entityIn.addEffect(new MobEffectInstance((Holder<MobEffect>) DysEffects.SENPAI_STARE, 1000, level + 1, true, true), entityIn);
                }
                if (level == 0) {
                ...
                if (level >= 1) {
                    entityIn.removeEffect((Holder<MobEffect>) DysEffects.SENPAI_STARE);
                    ...
                    if (SpawnUtil.trySpawnMob(EntityType.WARDEN, MobSpawnType.TRIGGERED, serverLevel, entityIn.blockPosition(), 20, 5, 6, SpawnUtil.Strategy.ON_TOP_OF_COLLIDER).isPresent()) {
                        ...
                        levelIn.playSound((Player) null, entityIn.getX(), entityIn.getY(), entityIn.getZ(), DysSounds.DEDEDON_SOUND, SoundSource.AMBIENT, 0.5f, 1f);
                    } else {
                        entityIn.sendSystemMessage(Component.translatable("msg.golden_ringo.3.1"));
                    }
                }
            } else {
                entityIn.addEffect(new MobEffectInstance((Holder<MobEffect>) DysEffects.SENPAI_STARE, 1000, 0, true, true), entityIn);
